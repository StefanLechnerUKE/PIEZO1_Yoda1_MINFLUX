...
% choose which data to analyse: 0 = P1-CTL, 1 = P1-cytoD, 2 = P1-cytoD+Yoda,
% 3 = A2094W-CTL, , 4 = A2094W-cytoD, 5 = A2094W-cytoD+yoda, 6 =
% VF1714_15AA-CTL, 7 = VF1714_15AA-cytoD, 8 = VF1714_15AA-cytoD+yoda
   ...
   Exclude = [8 13 15 24 33 62 73 85 106]     %%trimers excluded because the signals are not likely in the plasma membrane
    ...
    Exclude = [1 25 31 34 40 42 45 48 49 51 52 55 56 59 60 61 63 66 68 70 84 86 91 104 116 118 120]; %% trimers excluded because the signals are not likely in the plasma membrane

    elseif  DataSource == 2
    load("PIEZO1_ALFAmGL_CYTOD_Yoda_RawData.mat");
             myStruct = PIEZO1_ALFAmGL_CYTOD_Yoda_RawData;
             result.color = [1 0 0];
        Exclude = [5 6 24 38 39 71 101 103 104 112 113]'; %% trimers excluded because the signals are not likely in the plasma membrane 
   
    elseif  DataSource == 3
    load("A2094W_ALFAmGL_CTL_RawData.mat");
             myStruct = A2094W_ALFAmGL_CTL_RawData;
             result.color = [1 0.55 0];
        Exclude = [23 25 31 34 46 50 72 87 88 92]'; %% trimers excluded because the signals are not likely in the plasma membrane
   
    
    elseif  DataSource == 4
    load("A2094W_ALFAmGL_CYTOD_RawData.mat");
             myStruct = A2094W_ALFAmGL_CYTOD_RawData;
             result.color = [1 0.6 0];
        Exclude = [4 5 7 8 16 18 19 27 39 43 45 51 54 56 61 71 80 81 82 97 102 103 105]'; %% trimers excluded because the signals are not likely in the plasma membrane
   
        elseif  DataSource == 5
    load("A2094W_ALFAmGL_CYTOD_Yoda_RawData.mat");
             myStruct = A2094W_ALFAmGL_CYTOD_Yoda_RawData;
             result.color = [1 0.6 0.6];
        Exclude = [2 8 11 15 18 25 29 31 32 34 35 38 40 59 71 75 81 89]'; %% trimers excluded because the signals are not likely in the plasma membrane

    elseif  DataSource == 6
    load("VF1714_15AA_ALFAmGL_CTL_RawData.mat");
             myStruct = VF1714_15AA_ALFAmGL_CTL_RawData;
             result.color = [1 0.6 0.6];
        Exclude = [7 9 11 38 76 83 89 97 103 105 106 111 119 121 122]'; %% trimers excluded because the signals are not likely in the plasma membrane    
    
    elseif  DataSource == 7
    load("VF1714_15AA_ALFAmGL_CYTOD_RawData.mat");
             myStruct = VF1714_15AA_ALFAmGL_CYTOD_RawData;
             result.color = [1 0.6 0.6];
        Exclude = [7 11 12 14 21 26 32 34 36 43 45 51 52 55 56 59 63 65 66 80 83 86 92 102 105 106 107 109 114 116 119 120]'; %% trimers excluded because the signals are not likely in the plasma membrane

            elseif  DataSource == 8
    load("VF1714_15AA_ALFAmGL_CYTOD_Yoda_RawData.mat");
             myStruct = VF1714_15AA_ALFAmGL_CYTOD_Yoda_RawData;
             result.color = [1 0.6 0.6];
        Exclude = [12 26 28 38 75 93 94 95 97 100 102 107 109]'; %% trimers excluded because the signals are not likely in the plasma membrane
